 ...
 *
 * See rustRuntimeCalls.mjs, where the calls that rule governs live.
 *
 * ## Layout
 *
 * This file is the sidecar itself: spawning it, the protocol, supervision and readiness. What is built ON it
 * lives beside it — `agent.run` in rustRuntimeAgent.mjs, tool/process/sub-agent calls in rustRuntimeCalls.mjs,
 * the handshake's session policy in rustRuntimePolicy.mjs — and everything is imported through the facade,
 * rustRuntime.mjs, so no caller needs to know which file a function lives in.
 */
import { spawn } from "node:child_process";
import { recordRecovery } from "../store/recoveryLog.mjs";
import { runtimeBinaryPath } from "./runtimeBinary.mjs";
import { sessionPolicyParams } from "./rustRuntimePolicy.mjs";
...
/**
 * Where the compiled sidecar lives: packaged beside the app, or the newest cargo build during development.
 *
 * Read afresh on every call rather than cached: the tests point the bridge at a series of binaries within one
 * process, and a developer rebuilding mid-session should get the new one on the next spawn. The rule itself is
 * in runtimeBinary.mjs, shared with scripts/ensure-runtime.mjs so the pre-flight checks the binary the app will
 * actually run.
 */
function binaryPath() {
...
export function bridgeStatus() {
  return {
    ...
    /**
     * The roots this sidecar was confined to at its handshake, or `[]` when it runs unconfined.
     *
     * Reported rather than inferred: whether a command is sandboxed depends on what the HOST declared, so
     * reading the Rust side alone cannot answer it — which is exactly how the sandbox stayed inert unnoticed.
     */
    confinedTo: state?.ready ? [...(state.declaredRoots ?? [])] : [],
    /** Of the confinement, the roots that are readable but not writable. */
    readOnly: state?.ready ? [...(state.readonlyRoots ?? [])] : [],
...
export async function ensureStarted() {
  ...
  const s = {
    ...
    heartbeat: null,
    /// The roots declared at THIS handshake. The filesystem half of the ceiling is frozen once sent
    /// (see session_policy.rs), so this is the confinement for the whole life of this sidecar.
    declaredRoots: [],
    /// Of those, the ones a command may read but never write.
    readonlyRoots: [],
  };
  state = s;
  ...
  try {
    const policy = sessionPolicyParams();
    const init = await request(
      s,
      "runtime.initialize",
      { protocol_version: PROTOCOL_VERSION, client: "zeraix-electron", ...policy },
      INIT_TIMEOUT_MS,
    );
    ...
    // Said at the handshake because this is where confinement is decided for the whole session, and because
    // "is the sandbox on?" previously had no answer short of reading the runtime's own trace output.
    s.declaredRoots = policy.workspace_roots ?? [];
    s.readonlyRoots = policy.readonly_roots ?? [];
    if (s.declaredRoots.length || s.readonlyRoots.length) {
      console.info(
        `[rust-runtime] commands are confined to ${s.declaredRoots.join(", ") || "(no writable root)"} ` +
          (s.readonlyRoots.length ? `(read-only: ${s.readonlyRoots.join(", ")}) ` : "") +
          `plus each command's own working directory and the system toolchain; ` +
          `Landlock on Linux, lexical checks elsewhere`,
      );
    } else {
      console.warn(
        "[rust-runtime] no workspace roots were declared — commands run UNCONFINED. See setSessionPolicyProvider.",
      );
    }
    return s;
  } catch (e) {
