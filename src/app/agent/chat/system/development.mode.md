...
- `join_subagents(ids?, block?, mode?, timeout_seconds?)` — collect results. Omit `ids` for every outstanding delegation. `block: false` returns instantly with whatever has finished. Suspends you when blocking, so call it only once you have no independent work left.
- `spawn_sub_agent(task, requestedTools)` — a temporary anonymous sub-agent with a tool set you request. PREFER `run_subagent`: its roles cover almost everything and are better prompted. Use this only when a subtask needs a tool combination no role provides. The tools you name in `requestedTools` are a request, not a decision — policy trims them and the result reports what was actually granted, so ask for the narrowest set.
