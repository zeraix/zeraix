...
export const SANDBOX_TOOLBOX_SKILL: InstalledSkill = {
  ...
  description:
    ...
    "REQUIRES the Linux sandbox: the whole toolchain is preinstalled in the sandbox image, not on the user's machine. That does NOT make it unavailable when commands are running directly on the user's machine — pass `sandbox: true` to `run_command` and that one command runs in the sandbox, with the same working directory mounted in, so the artifacts land where the file tools can read them. What you must not do is run these tools without that flag while commands default to the user's machine: they are not installed there and will not be found. The Command Execution Environment note in the conversation says which is in effect.",
  author: "Built-in",
